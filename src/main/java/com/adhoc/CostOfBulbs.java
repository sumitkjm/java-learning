...
public class CostOfBulbs {
    ...
    public static void main(String[] args) {
        ...
        String[] pat = in.nextLine().split(" ");
        int n = Integer.valueOf(pat[0]);
        long x = Long.valueOf(pat[1]);
        long y = Long.valueOf(pat[2]);
        String bulbs = in.nextLine();
        long cost = 0;
        int groups = 0;
        boolean firstZero = false;
        for(int i=0;i<n;i++) {
            if(firstZero && i>0 && bulbs.charAt(i)=='0' && bulbs.charAt(i-1)=='1') {
                groups++;
            }
            if(!firstZero && bulbs.charAt(i)=='0') {
                firstZero = true;
            }
        }
        if(x<y) {
