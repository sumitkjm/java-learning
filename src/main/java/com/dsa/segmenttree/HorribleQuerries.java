...
//            System.out.println("low>high,low="+low+",high="+high);
        ...
        if(lazy[treeIndex]!=0) {
            ...
            if (low != high) {
                int mid = (low + high) / 2;
                lazy[2 * treeIndex] += (mid - low + 1) * lazy[treeIndex] / (high - low + 1);
                lazy[2 * treeIndex + 1] += (high - mid) * lazy[treeIndex] / (high - low + 1);
            }
            lazy[treeIndex] = 0;
        }
        // no overlap
        ...
        // complete overlap
            if(startR<=low&& high<=endR) {
                ...
                if(low != high) {
                    int mid = (low+high)/2;
                    lazy[2*treeIndex] += (mid-low+1)*inc;
                    lazy[2*treeIndex+1] +=(high-mid)*inc;
                }
                return;
            }
        // partial overlap
