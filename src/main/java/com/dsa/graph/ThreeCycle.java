...
public class ThreeCycle {
    ...
    public static int solve(boolean[][] graph, int n) {
        ...
        for (int i=0;i<n;i++) {
            for (int j=0;j<n;j++) {
                // if same node then continue or i, j does not have edge then also continue
                if(i==j||!graph[i][j]) {
                    continue;
                }
                for (int k=0;k<n;k++) {
                    // j, k and k and i should not be same
                    // j, k should have edge
                    if(j==k || k==i||!graph[j][k]) {
                        continue;
                    }
                    // it reaches here that means j, k have edge so check if i and k also
                    // has an edge if yes then increase the count.
                    if(graph[i][k]) {
                        count++;
                    }
                }
        ...
        // answer needs to diveded by 6 as duplicates the counting for each node in 3 cycle
        // from each node it counts twice so there are 3 nodes so 3*2 times more
        return count/6;
    }
